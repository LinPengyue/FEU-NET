...
class ImageLoader_pc(torch.utils.data.Dataset):
    def __init__(self, root, transform=None, split="train", loader=pil_loader):

        self.imgs_data_folder = os.path.join(root, "VG_Annotations", "imgs_data.pickle")
        self.splits_folder = os.path.join(root, "VG_Annotations", "data_splits.pickle")
        # self.annotations_folder = os.path.join(root, "VG_Annotations", "region_descriptions.json")
        self.annotations_folder = "/data2/lpy1/mllms_know-main/VG_region_descriptions_pfclean_cfclean.json"
        with open(self.annotations_folder, 'rb') as f:
            # self.annotations = json.load(f, encoding='latin1')
            self.annotations = json.load(f)
        with open(self.splits_folder, 'rb') as f:
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            region_id = np.random.randint(0, len(ann))
            return img, 'image of ' + ann[region_id]['phrase'].lower(), 'image of ' + ann[region_id]['paraphrase'].lower(), 'image of ' + ann[region_id]['counter'].lower()
        out = {}
        for i in range(0, len(ann)):
            tmp = {}
            tmp['sentences'] = 'image of ' + ann[i]['phrase'].lower()
            tmp['paraphrase'] = 'image of ' + ann[i]['paraphrase'].lower()
            tmp['counter'] = 'image of ' + ann[i]['counter'].lower()
            bbox = [[int(ann[i]['x']), int(ann[i]['y']),\
                   int(ann[i]['x']) + int(ann[i]['width']), int(ann[i]['y']) + int(ann[i]['height'])]]
            tmp['bbox'] = bbox
            if (bbox[0][3] - bbox[0][1]) * (bbox[0][2] - bbox[0][0]) > 0.05 * image_sizes[0] * image_sizes[1]:
        ...
        #     tmp = {}
        #
        #     # 使用 paraphrase，如果存在；否则 fallback 到 phrase
        #     paraphrase_text = ann[i].get('paraphrase', '').strip()
        #     if not paraphrase_text:  # 如果 paraphrase 为空，使用 phrase
        #         paraphrase_text = ann[i].get('phrase', 'a region').strip()
        #
        #     # 构造句子，加上 "image of" 前缀并转为小写
        #     tmp['sentences'] = 'image of ' + paraphrase_text.lower()
        #
        #     # 生成 bounding box [x1, y1, x2, y2]
        #     bbox = [
        #         [
        #             int(ann[i]['x']),
        #             int(ann[i]['y']),
        ...
        #
        #     # 可选：保留原始字段（便于调试）
        #     # tmp['phrase'] = ann[i]['phrase']
        #     # tmp['paraphrase'] = ann[i]['paraphrase']
        #
        #     # 条件过滤：bbox 面积大于图像总面积的 5%
        #     bbox_area = (bbox[0][3] - bbox[0][1]) * (bbox[0][2] - bbox[0][0])
        #     image_area = image_sizes[0] * image_sizes[1]  # 假设 image_sizes = [height, width]
...
def get_VG_dataset_pc(args):
    datadir = args['data_path']
    transform_train, transform_test = get_flicker_transform(args)
    ds_train = ImageLoader_pc(datadir, split='train', transform=transform_train)
    return ds_train
...
def get_VGtest_dataset_pc(args):
    datadir = args['val_path']
    transform_train, transform_test = get_flicker_transform(args)
    ds_test = ImageLoader_pc(datadir, split='test', transform=transform_test)
    return ds_test
